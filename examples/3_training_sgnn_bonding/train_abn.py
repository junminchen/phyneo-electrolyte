...
DEFAULT_MONOMERS = ["conf_18_DFP", "conf_20_PF6", "conf_15_BF4"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train the ABn sGNN bonding model.")
    parser.add_argument(
        "--dataset",
        action="append",
        dest="datasets",
    ...
    parser.add_argument(
        ...
        dest="monomers",
        default=[],
        help="Training monomer key. Can be passed multiple times.",
    )
    ...
    parser.add_argument(
        "--params-out",
        type=Path,
        default=script_dir / "params_sgnn_ABn.pickle",
        help="Path to save the best ABn parameters.",
    )
    parser.add_argument(
        "--restart",
        type=Path,
    ...
    parser.add_argument(
        "--log-file",
        type=Path,
        default=script_dir / "nn_ABn.err",
        help="Training log output.",
    )
    parser.add_argument(
        "--test-data-out",
        type=Path,
        default=script_dir / "test_data_ABn.xvg",
    ...
    if not args.monomers:
        args.monomers = list(DEFAULT_MONOMERS)
    return args
...
def make_train_batches(data: dict, batch_size: int) -> list[tuple[jnp.ndarray, jnp.ndarray]]:
...
def main() -> int:
    ...
    spec = resolve_default_sgnn_specs()["abn"]
    ...
    for key in args.monomers:
        ...
        def mse_loss(params, positions, box, ene_ref, key_=key):
            ene = cal_energy[key_](positions, box, params)
            err = ene - ene_ref
            err -= jnp.average(err)
    ...
    if model is None:
        raise RuntimeError("No models were initialized. Check the monomer list.")
    ...
    with open(args.log_file, "w") as fout:
        fout.write("Sub-Graph Neural Network Package used for intramolecular energy\n")
        ...
        for i_epoch in range(args.epochs):
            ...
            for key, pos, ene_ref in trunk_train:
                loss, gradients = mse_loss_grad[key](params, pos, box, ene_ref)
                lossprop += loss
            ...
            if lossprop < best_loss:
                with open(args.params_out, "wb") as f:
