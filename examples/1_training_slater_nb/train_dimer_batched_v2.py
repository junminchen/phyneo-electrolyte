#!/usr/bin/env python
"""Train Slater short-range parameters using full-batch parallel evaluation."""
...
# Ensure weights are balanced or configurable
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Batched Train Slater dimer parameters (V2)")
    ...
    parser.add_argument("--out", default=str(script_dir / "params" / "params_ff_batched_v2.pickle"))
    ...
    parser.add_argument("--log-file", default=str(script_dir / "output" / "train_batched_v2.log"))
...
def main():
    ...
    for pair in data.keys():
        if pair in dimer_repulsive:
            ...
            for sid in data[pair].keys():
                data[pair][sid]["wts"] = calculate_weights(jnp.asarray(data[pair][sid]["tot_full"]), args.thresh)
    ...
    for class_name, class_instance in class_instances.items():
        # Double vmap: over batches, then over frames
        cal_energy[class_name] = jit(vmap(vmap(class_instance.cal_e, in_axes=(None, 0, 0), out_axes=(0, 0, 0, 0, 0, 0)), in_axes=(None, 0, 0), out_axes=(0, 0, 0, 0, 0, 0)))

    processed_data = {}
    for key in dimer_train:
        # Balanced subset for final convergence
        batch_keys = sorted(data[key].keys())
        subset_keys = batch_keys[:10] 
        processed_data[key] = {
            "posA": jnp.stack([jnp.asarray(data[key][b]["posA"]) for b in subset_keys]),
            "posB": jnp.stack([jnp.asarray(data[key][b]["posB"]) for b in subset_keys]),
            "wts": jnp.stack([jnp.asarray(data[key][b]["wts"]) for b in subset_keys]),
            "ex": jnp.stack([jnp.asarray(data[key][b]["ex"]) for b in subset_keys]),
            "es": jnp.stack([jnp.asarray(data[key][b]["es"]) for b in subset_keys]),
            "pol": jnp.stack([jnp.asarray(data[key][b]["pol"]) for b in subset_keys]),
            "disp": jnp.stack([jnp.asarray(data[key][b]["disp"]) for b in subset_keys]),
            "dhf": jnp.stack([jnp.asarray(data[key][b]["dhf"]) for b in subset_keys]),
            "tot": jnp.stack([jnp.asarray(data[key][b]["tot"]) for b in subset_keys]),
        }
        logger.info(f"Pair {key}: {len(subset_keys)} batches aggregated (out of {len(batch_keys)}).")
    ...
    @jit
    def full_batched_loss(params_local, global_data):
        total_loss = jnp.float32(0.0)
        pair_losses = {}
        
        for pair_key in dimer_train:
            p_data = global_data[pair_key]
            weights_pts = p_data["wts"] # (N_batches, 12)
            e_ex, e_es, e_pol, e_disp, e_dhf, e_tot = cal_energy[pair_key](
                params_local, p_data["posA"], p_data["posB"]
            )
            # Shapes: (N_batches, 12)
            
            # Combine components
            pred = jnp.stack([e_ex, e_es, e_pol, e_disp, e_dhf, e_tot], axis=0) # (6, N_batches, 12)
            ref = jnp.stack([p_data[c] for c in COMPONENTS], axis=0) # (6, N_batches, 12)
            
            # Calculate loss for this pair
            norm = jnp.sum(weights_pts)
            # squared error weighted by points weights, then normalized
            errs = jnp.sum((ref - pred) ** 2 * (weights_pts[None, :, :] / norm), axis=(1, 2)) # (6,)
    ...
    batched_loss_grad = jit(value_and_grad(full_batched_loss, has_aux=True))

    @jit
    def train_step_fn(params_local, opt_state_local, global_data):
        (loss, pair_losses), grads = batched_loss_grad(params_local, global_data)
        ...
        return safe_params, safe_opt_state, loss, pair_losses, is_finite
    ...
    logger.info("JIT Compiling full-batched loss (may take a minute)...")
    init_loss, init_pair_losses = full_batched_loss(params, processed_data)
    logger.info(f"Compilation done. Initial full-batched loss: {float(init_loss):.6f}")

    for i_epoch in range(args.epochs):
        params, opt_state, global_loss, pair_losses, is_finite = train_step_fn(
            params, opt_state, processed_data
        )
        ...
        if i_epoch % args.log_every == 0:
            ...
            logger.info(
                f"epoch={i_epoch:04d}  avg={float(global_loss):.4f}  time={elapsed_epoch:.1f}s  finite={bool(is_finite)}\n"
                f"  [{pair_str}]",
            )
    ...
    logger.info(f"Saved FF-tree full-batched params to: {out_path}")
