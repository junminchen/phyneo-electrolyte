...
def _load_yaml_config(config_path: str, script_dir: Path) -> dict:
    """Load YAML config and resolve relative paths w.r.t. script_dir."""
...
def parse_args():
    ...
    parser.add_argument("--config", default=None, metavar="YAML",
                        help="Path to train_config.yaml; CLI args override YAML values")
    ...
    parser.add_argument("--log-file", default=str(script_dir / "output" / "train_backend.log"))
    parser.add_argument("--debug-nans", action="store_true")
    parser.add_argument("--freeze-b", action="store_true")

    # ── load YAML first so it becomes the default for any unset CLI arg ──────
...
def main():
    ...
    for i_epoch in range(args.epochs):
        ...
        pair_losses: dict[str, list[float]] = {k: [] for k in dimer_train}
        for key0, batch in trunk:
            params, opt_state, loss = train_step[key0](params, opt_state, processed_data[key0][batch])
            pair_losses[key0].append(float(loss))

        if i_epoch % args.log_every == 0:
            avg_losses = {k: float(np.mean(v)) for k, v in pair_losses.items() if v}
            total_avg = float(np.mean(list(avg_losses.values())))
            elapsed_epoch = time.time() - t_epoch
            t_epoch = time.time()
            pair_str = "  ".join(f"{k.split('_', 2)[2]}={v:.3f}" for k, v in avg_losses.items())
            logger.info(
                f"epoch={i_epoch:04d}  avg={total_avg:.4f}  time={elapsed_epoch:.1f}s\n"
                f"  [{pair_str}]",
            )
    ...
    logger.info(f"Saved FF-tree params to: {out_path}")
