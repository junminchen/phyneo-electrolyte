"""
Benchmark: sGNNForceFast (sparse gather) in OpenMM via openmmtorch TorchForce.
"""
import warnings
...
class GraphAdapter:
    def __init__(self, G):
        self.bonds = G.bonds; self.b0 = G.b0; self.fscale_bond = FSCALE_BOND
        self.angles = G.angles; self.cos_a0 = G.cos_a0; self.fscale_angle = FSCALE_ANGLE
        self.diheds = G.diheds; self.feature_atypes = G.feature_atypes
        self.feature_indices = G.feature_indices; self.nb_connect = G.nb_connect
        self.weights = G.weights; self.n_features = int(G.n_features)
...
def run_benchmark(warmup=500, bench=2000):
    ...
    t0 = time.time()
    box = torch.tensor([[BOX_SIZE,0,0],[0,BOX_SIZE,0],[0,0,BOX_SIZE]], dtype=torch.float32)
    G = from_pdb(PDB_FILE)
    G.set_box(box)
    _ = MolGNNForce(G=G, nn=1)
    print(f"   {G.n_atoms} atoms, {len(G.bonds)} bonds, {len(G.subgraphs)} subgraphs in {time.time()-t0:.1f}s")
    ...
    # 3. Standalone PyTorch benchmark
    ...
    # 4. JIT trace & save
    print("4. JIT tracing model...")
    class Wrapper(nn.Module):
        def __init__(self, core):
            super().__init__()
            self.core = core
        def forward(self, positions: torch.Tensor, boxvectors: torch.Tensor) -> torch.Tensor:
            return self.core(positions.float() * 10.0, boxvectors.float() * 10.0) * 4.184

    wrapped = Wrapper(model)
    pos_nm = (G.positions.clone() / 10.0).to(DEVICE)
    box_nm = (box / 10.0).to(DEVICE)
    traced = torch.jit.trace(wrapped, (pos_nm, box_nm))
    fd, temp_path = tempfile.mkstemp(suffix='.pt')
    os.close(fd)
    traced.save(temp_path)

    # 5. Create OpenMM system
    ...
    # Remove classical bonded forces
    to_rm = [i for i in range(system.getNumForces())
             if system.getForce(i).__class__.__name__ in
    ...
    integrator = mm.LangevinIntegrator(298*unit.kelvin, 1.0/unit.picoseconds, 1.0*unit.femtosecond)
    ...
    if ns_day >= 15:
        ...
        print(f"  Estimated 10k:    ~{est_10k:.1f} ns/day {'PASSED' if est_10k >= 15 else 'NEEDS WORK'}")
